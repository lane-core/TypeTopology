...
module Base.Subsingletons where
...
singleton-type x = Σ y ꞉ object x , x ＝ y
...
singleton-types-are-singletons'' {𝓤} {X} = 𝕁 A (λ x → refl)
 where
  A : (x x' : X) → x ＝ x' → 𝓤 ̇
...
singleton-type' x = Σ y ꞉ object x , y ＝ x
...
subtypes-of-props-are-props' : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } (m : X → Y)
                             ...
                             → is-prop Y
                             → is-prop X
subtypes-of-props-are-props' m lc i x x' = lc x x' (i (m x) (m x'))

pr₁-lc : {X : 𝓤 ̇ } {Y : X → 𝓥 ̇ }
       → ({x : X} → is-prop (Y x))
       → Left-Cancellable (pr₁ {𝓤} {𝓥} {X} {Y})
pr₁-lc {𝓤} {𝓥} {X} {Y} f p q i = to-Σ-＝ (i , (f (transport Y i (pr₂ p)) (pr₂ q)))
