...
section-of : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } → (X → Y) → 𝓤 ⊔ 𝓥 ̇
section-of r = Σ s ꞉ (codomain r → domain r), r ∘ s ∼ id
...
retraction-of : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } → (X → Y) → 𝓤 ⊔ 𝓥 ̇
retraction-of s = Σ r ꞉ (codomain s → domain s), r ∘ s ∼ id
...
section-retraction-of : {X : 𝓤 ̇ } {Y : 𝓥 ̇ }
                   → (ρ : retract X of Y)
                   → retraction-of (section ρ)
section-retraction-of (r , s , rs) = r , rs

retract-condition : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } (ρ : retract X of Y)
                  → retraction ρ ∘ section ρ ∼ id
retract-condition (r , s , rs) = rs

retraction-section-of : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } (ρ : retract X of Y)
                       → section-of (retraction ρ)
retraction-section-of (r , h) = h
...
section-of-closed-under-∼ : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } (f g : X → Y)
                           → section-of f
                           → g ∼ f
                           → section-of g
section-of-closed-under-∼ {𝓤} {𝓥} {X} {Y} f g (s , fs) h =
...
section-of-closed-under-∼' : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } {f g : X → Y}
                            ...
                            → f ∼ g
                            → section-of g
section-of-closed-under-∼' ise h =
 section-of-closed-under-∼ _ _ ise (λ x → (h x)⁻¹)

retraction-of-closed-under-∼ : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } (f g : X → Y)
                          → retraction-of f
                          →  g ∼ f
                          → retraction-of g
retraction-of-closed-under-∼ {𝓤} {𝓥} {X} {Y} f g (r , rf) h =
...
retraction-of-closed-under-∼' : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } {f g : X → Y}
                           ...
                           → f ∼ g
                           → retraction-of g
retraction-of-closed-under-∼' ise h =
 retraction-of-closed-under-∼ _ _ ise (λ x → (h x)⁻¹)
...
section-of' : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } (f : X → Y) → 𝓤 ⊔ 𝓥 ̇
section-of' f = (y : codomain f) → Σ x ꞉ domain f , f x ＝ y

retract_Of_ : 𝓤 ̇ → 𝓥 ̇ → 𝓤 ⊔ 𝓥 ̇
retract Y Of X = Σ f ꞉ (X → Y) , section-of' f
...
ap-of-section-retraction-of : {X : 𝓤 ̇ } {Y : 𝓥 ̇ } (s : X → Y)
                         → retraction-of s
                         → (x x' : X) → retraction-of (ap s {x} {x'})
ap-of-section-retraction-of {𝓤} {𝓥} {X} {Y} s (r , rs) x x' = ρ , ρap
 where
  ρ : s x ＝ s x' → x ＝ x'
  ρ q = x        ＝⟨ (rs x) ⁻¹ ⟩
        r (s x)  ＝⟨ ap r q ⟩
  ...
  where
   ...
   γ x = ρ , (σ , ρσ)
    where
     ...
     ρ : s (g x) ＝ s y → g x ＝ y
     ρ = pr₁ (ap-of-section-retraction-of s (r , rs) (g x) y)

     ρσ : (p : g x ＝ y) → ρ (σ p) ＝ p
     ρσ = pr₂ (ap-of-section-retraction-of s ((r , rs)) (g x) y)
